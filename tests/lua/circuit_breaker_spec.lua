describe("Phase 2 access policy", function()
    ...
    it("validates the staged MCP compatibility token", function()
        assert.truthy(policy:find("PROVOST_TOKEN", 1, true))
        assert.truthy(policy:find("X-Provost-Token", 1, true))
        assert.truthy(policy:find("PROVOST_TOKEN_INVALID", 1, true))
    end)
end)