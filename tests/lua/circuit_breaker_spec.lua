describe("Phase 2 access policy", function()
    ...
    it("routes named LLM backends and rejects unknown names", function()
        assert.truthy(policy:find('os.getenv("LLM_ROUTES_JSON")', 1, true))
        assert.truthy(policy:find("cjson.decode", 1, true))
        assert.truthy(policy:find("ngx.var.backend_name", 1, true))
        assert.truthy(policy:find("Unknown LLM backend:", 1, true))
        assert.truthy(policy:find("ngx.HTTP_NOT_FOUND", 1, true))
    end)
