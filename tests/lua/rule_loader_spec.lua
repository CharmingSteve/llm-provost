...
describe("rule_loader: file loading", function()
    ...
    it("on-disk rules.json contains tool_allowlist", function()
        local content, _ = read_file("rules.json")
        local rules, _   = parse_and_validate(content)
        assert.is_table(rules.tool_allowlist)
    end)

    it("on-disk rules.json contains rate_limits", function()
        local content, _ = read_file("rules.json")
        local rules, _   = parse_and_validate(content)
        assert.is_table(rules.rate_limits)
    end)

    it("on-disk rules.json contains time_based_rules", function()
        local content, _ = read_file("rules.json")
        local rules, _   = parse_and_validate(content)
        assert.is_table(rules.time_based_rules)
    end)
