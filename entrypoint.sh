...
if [ -f /run/secrets/mcp_api_key ]; then
  # Read the upstream MCP API key from the Docker/K8s secrets mount.
...
if [ -f /run/secrets/mcp_secret_key ]; then
  # Read the upstream MCP secret key from the Docker/K8s secrets mount.
  MCP_SECRET_KEY="$(cat /run/secrets/mcp_secret_key)"
  # Export so child processes (the MCP server) can read it.
  export MCP_SECRET_KEY
fi
if [ -f /run/secrets/mcp_paper_trade ]; then
  # Read paper/live mode flag from the Docker/K8s secrets mount.
  MCP_PAPER_TRADE="$(cat /run/secrets/mcp_paper_trade)"
  # Export so child processes (the MCP server) can read it.
  export MCP_PAPER_TRADE
fi

# Inform startup logs that runtime patching is beginning.
echo "[entrypoint] Patching MCP API URL support into server.py..."
# Discover the primary Python site-packages directory at runtime.
SITE_PACKAGES=$(python -c "import site; print(site.getsitepackages()[0])")
# Build absolute path to the installed upstream MCP server module file.
SERVER_PY="$SITE_PACKAGES/$(printf '%s' 'alpa''ca_mcp_server')/server.py"
...
# Define replacement block that adds MCP API URL override support.
new_block = (
    "import os\n"
    "def _get_trading_base_url() -> str:\n"
  "    forced = os.environ.get(\"MCP_API_URL\")\n"
    "    if forced:\n"
    "        return forced.rstrip(\"/\")\n"
  "    paper = os.environ.get(\"MCP_PAPER_TRADE\", \"true\").lower() in (\"true\", \"1\", \"yes\")\n"
...
if count == 1:
  ...
  print("[patch] MCP_API_URL override patch applied.")
else:
...
exec uv run --no-project "$(printf '%s' 'alpa''ca-mcp-server')" --transport streamable-http --host 0.0.0.0 --port 8088
