-- alpaca_policy.lua
-- Enforce REST endpoint governance for outbound MCP->API traffic (hop 2),
-- ported from Agent Provost's http_policy.lua. Applies the per-server
-- trading rules from rules.json (mcp_servers.alpaca) via trading_rules.lua.

local cjson = require("cjson.safe")
local engine = require("trading_rules")
...
local function read_rules()
    ...
    -- Prefer the per-server Alpaca rule set; fall back to the root table
    -- for compatibility with flat (Agent Provost style) rules files.
    if type(decoded.mcp_servers) == "table"
       and type(decoded.mcp_servers.alpaca) == "table" then
        return decoded.mcp_servers.alpaca
    end
    return decoded
end
...
-- 429 reject used when the upstream rate-limit budget is exhausted.
function _M.reject_upstream_budget(error_code, detail)
    local audit_error = require("audit_error")
    audit_error.emit("provost_api_to_mcp_error", 429, error_code, detail, {
        request_id = ngx.var.provost_req_id,
    })
    ngx.status = 429
    ngx.header["Content-Type"] = "application/json"
    local body = cjson.encode({
        error = error_code,
        detail = detail or "Upstream rate-limit budget exhausted"
    })
    ngx.var.resp_body = body or ""
    ngx.say(body)
    return ngx.exit(429)
