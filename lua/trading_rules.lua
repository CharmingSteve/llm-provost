-- trading_rules.lua
-- Trading-domain rule evaluation, ported from Agent Provost's rules_engine.
--
-- Evaluates a decoded MCP tools/call body (or a raw HTTP method+path for the
-- outbound MCP-to-API hop) against a per-MCP-server rules table. This module
-- has no I/O or OpenResty dependencies and can be required in both OpenResty
-- workers and busted unit tests.
--
-- Supports both the original Agent Provost rule names and generic aliases:
--   max_notional        -> max_trade_notional
--   share_limits        -> max_trade_size
--   symbol_allowlist    -> allowed_tickers
--   symbol_blocklist    -> blocked_tickers
--   forbidden_endpoints -> forbidden_tools
--
-- Usage:
--   local trading = require("trading_rules")
--   local blocked, reason = trading.check_request(parsed, server_rules, context)
--   if blocked then ... end

local _M = {}
local cjson = require("cjson.safe")

-- Generic alias -> Agent Provost canonical rule name.
local RULE_ALIASES = {
    max_notional = "max_trade_notional",
    share_limits = "max_trade_size",
    symbol_allowlist = "allowed_tickers",
...
-- Returns a shallow copy of rules with alias keys mapped to canonical names.
-- Canonical keys win when both are present.
local function resolve_rule_aliases(rules)
    ...
    for alias, canonical in pairs(RULE_ALIASES) do
        if resolved[canonical] == nil and resolved[alias] ~= nil then
...
_M.resolve_rule_aliases = resolve_rule_aliases
...
function _M.check_request(parsed, rules, context)
    rules = resolve_rule_aliases(rules)
